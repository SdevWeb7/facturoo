...
import { JournalVentesDocument } from "@/components/pdf/JournalVentesDocument";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const buffer = await renderToBuffer(
      createElement(JournalVentesDocument, docProps) as ReactElement<any>
    );

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="journal-ventes.pdf"`,
      },
    });
  } catch (error) {
